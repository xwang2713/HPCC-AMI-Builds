...
# Turning on logging
exec > >(sudo sh -c "tee >> /var/log/ami-one-log-$codename-$now.log|logger -t ami-build -s 2>/dev/console") 2>&1
...
export EC2_HOME_REGION=eu-central-1
#export EC2_HOME_REGION=eu-west-1
...
then
	ec2-upload-bundle --batch --bucket $EC2_HOME_S3BUCKET --region $EC2_HOME_REGION  --access-key $ACCESS_KEY --secret-key $SECRET_KEY --manifest ~/bundle/$bundlefn/$bundlefn.manifest.xml --retry
	touch ~/$bundlefn.uploaded.$EC2_HOME_REGION
fi

if [ ! -f ~/$bundlefn.registered.${EC2_HOME_REGION} ];
then
...
do 
	echo "Migrating $EC2_HOME_REGION image to AWS region [$i] of [${EC2_REGION_COUNT}]:  ${REGIONS[$i]}: ${S3BUCKETS[$i]}"
        [ "${REGIONS[$i]}" != "eu-central-1" ] && continue
	...
	then
		ec2-upload-bundle --batch --bucket ${S3BUCKETS[$i]} --access-key $ACCESS_KEY --secret-key $SECRET_KEY --manifest ~/bundle/$bundlefn/$bundlefn.manifest.xml --retry
